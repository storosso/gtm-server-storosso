  ...
  if (pathname === '/collect' || pathname === '/g/collect') {
    ...
    req.on('end', () => {
      try {
        ...
        const rawItems = parsed.custom_data?.contents || parsed.ecommerce?.add?.products || []
        ...
        const value = parsed.custom_data?.value || contents.reduce((sum, c) => sum + c.quantity * c.item_price, 0)
        const currency = parsed.custom_data?.currency || parsed.ecommerce?.currencyCode || 'EUR'
        const custom_data = { value, currency, content_ids, contents }
        const user_data = {
        ...
        const payload = { data: [{ event_name, event_time, event_source_url, action_source, user_data, custom_data }] }
        const options = {
        ...
        fbReq.on('error', () => res.writeHead(500).end())
